...
task :syncing_korona_po => :environment do
  if Date.today.wday == 2 && (3..11).to_a.include?(Time.now.utc.hour)
    next
  end
  # byebug
  ...
  if !get_store_order.nil? 
    ...
    get_store_order["results"].each do |order|
      ...
      if get_store_order_items
        get_store_order_items_collection << get_store_order_items
      else
        puts "The store with id: #{storeOrderId} and Number: #{order["number"]}, does not have any items. So, first add them then continue."
        # byebug if get_store_order["results"].count == new_store_count
        next 
      end
              ...
              matched_korona_product.each do |m_product|
                if !m_product["codes"].nil?
                  m_product["supplierPrices"].each do |code|
                    order_code = code["orderCode"].strip
                    matching_product = product_payload_variants.find { |p| p["sku"].include?(order_code) }
                    product_payload_variant = product_payload_variants.select{ |p| p["sku"].strip == order_code }
                    byebug
                    if product_payload_variant.any?
                      ...
                      puts "product not found (because Korona product with sku: #{order_code} not found)"
                      d = apiDel("https://167.koronacloud.com/web/api/v3/accounts/#{koronaAccountId}/storeOrders/#{storeOrderId}/items/#{productId}")
                      puts "deleted #{d}"
                    end    
                  end
      ...
      apiPost("https://tvs-stores-po-app.herokuapp.com/hooks/korona_order_create_hook", JSON.dump(payload)) if !payload.empty?
    ...
    apiPost("https://tvs-stores-po-app.herokuapp.com/hooks/korona_new_product_check_hook", JSON.dump(new_product_payload))
